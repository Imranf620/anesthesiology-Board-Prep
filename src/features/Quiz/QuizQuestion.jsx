...
const QuizQuestion = ({ toggleSide,userTestID }) => {
  ...
  const question = state.questions.find(ques => ques.QuestionID === activeQuestion);
  const [selectedRating, setSelectedRating] = useState(null);

  const handleCopyClick = () => {
    navigator.clipboard.writeText(`${question.Chapter} / QuestionId - ${question.QuestionID}`)
      .then(() => {
  ...
  const handleThumbClick = async rating => {
    try {
      ...
      toast.error('Failed to submit rating. Please try again.', { autoClose: 2000 });
    }
  };
  ...
  const handleMarkChange = e => {
    ...
    markQuestion(question.QuestionID, e.target.checked ? 'Y' : 'N', question.Correct);
  };
  ...
  useEffect(() => {
    const optExists = state.options.find(opt => opt.questionId === activeQuestion);
    ...
    <section className="mt-2 rounded-lg border-2 border-gray-500 bg-white shadow-md">
      <div className="flex flex-col gap-3 p-2 text-center md:p-5 lg:p-12">
        {state?.status?.toLowerCase() !== 'completed' && (
          <div className={`flex items-center justify-between gap-2 ${!activeOption ? 'opacity-60' : ''}`}>
            <div className="flex items-center gap-2">
              <div
                className="relative tool cursor-pointer rounded bg-[#fff] p-1 ring-1 ring-black duration-300 hover:bg-gray-300"
                onClick={handleBookmarkClick} // Add the click handler here
              >
            ...
            <ul className="flex flex-col gap-3">
              {question?.choices.map(choice => (
                ...
                >
                  ...
                  >
                    <span>{choice.OptionName}</span> <span>{choice.OptionStatement}</span>
                  </label>
                </li>
              ))}
            </ul>
